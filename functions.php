...
if ( ! function_exists( 'college_setup' ) ) :
	...
	function college_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'college' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'college_custom_background_args',
				array(
 ...
 */
function college_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'college_content_width', 640 );
}
add_action( 'after_setup_theme', 'college_content_width', 0 );
...
function college_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'college' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'college' ),
